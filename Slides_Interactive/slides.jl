...
# ╔═╡ eb5b60d8-deb4-11ed-2a2e-1527f08ac187
begin
...
# ╔═╡ 4220df1b-8033-4d74-97d3-913cf267060a
md"# Setup"

# ╔═╡ ccb385b5-cd53-4f97-a57e-541686b6cb0c
html"<button onclick='present()'>present</button>"

# ╔═╡ d846c343-e6cf-4ffa-a1a1-21ff658bea1b
begin
	global plots = Dict()
...
# ╔═╡ ec80bdfe-4274-4295-b4b3-b2fe0cf4c9fa
md"# Numerical Solution of the DE"

# ╔═╡ 222f2bbd-531a-4254-8eeb-1442a7239742
md"""
```math
...
# ╔═╡ a9d65758-be25-49df-b368-5bad7a7d99ad
tspan = (0.0, 1.0)

# ╔═╡ 4d9adf79-6c23-4a77-8717-1539ee086e27
ε = 0.1

# ╔═╡ 65ca7ae6-efc3-45a0-8c9d-ad07344a5d64
md"## Defining the DE for Solvers"

# ╔═╡ 512305ac-861f-45d0-bd74-ae60fe0824e8
"""
	boundary_layer_problem!(du, u, p, x; ϵ = 0.1)

The phase space function of the ODE problem
"""
...
# ╔═╡ bf584632-afd0-4de3-955d-c0fb797fad00
md"""
!!! note
	The definition that is given to the solvers is similar in how we would define the phase portrait variables... This approach would work for higher order differential equations.
"""

# ╔═╡ 734845d7-544f-4b18-a1e0-307b7af3e162
md"## Shooting Solver with Predetermined Initial Conditions"

# ╔═╡ bbc0ed4f-ccf6-4918-84d0-b22539d0c094
"""
	problem_ODE(u0)
	
Simple ODE problem with a predefined initial value
"""
	problem_ODE(u0) = ODEProblem(boundary_layer_problem!,
		u0,
		tspan)

# ╔═╡ 98335a2c-5883-4be1-bbc5-c555d3b390ee
md"## Numerical solution by shooting with predetermined initial slopes"

# ╔═╡ 1b7a159f-d445-4b58-8802-72c17ac1549b
begin
	B1 = solve(problem_ODE([1.0, 0.99990238]), Tsit5(), reltol = 1e-8, abstol = 1e-10);
	M = solve(problem_ODE([1.0, 0.9051]), Tsit5(), reltol = 1e-8, abstol = 1e-10)
	B0 = solve(problem_ODE([1.0, -10.6942]), Tsit5(), reltol = 1e-8, abstol = 1e-10)
end;

# ╔═╡ 5fa8f503-8013-4036-9945-cec558b18b5d
function figure_numerical_solutions()
	local f = Figure()
	local ax = Axis(f[1,1], title = L"Numerical solutions $B0$, $M$ and $B1$ with $\varepsilon = 0.1$", xlabel = L"x", ylabel = L"y")
	for (sol, name) in [(B0, L"y_\text{B0}(x)") , (M, L"y_\text{M}(x)"), (B1, L"y_\text{B1}(x)")]
		lines!(ax, sol.t, sol[1,:], label = name)
	end
	axislegend(ax)
...
# ╔═╡ 0c0fb0a4-7b3e-4456-9d93-4b9c91c69c61
with_backend(figure_numerical_solutions, CairoMakie)

# ╔═╡ 212a47bb-a3d8-43dd-9322-72c4ed24e9c7
with_backend(GLMakie) do
	f = figure_numerical_solutions()
	ax = current_axis()
	ax.aspect[] = DataAspect()
	f
end

# ╔═╡ 04edaf90-8ac5-4f0a-b537-cc89bfe9d1e7
md"## A Naive Solver"

# ╔═╡ 159b49e5-2c3c-411e-9203-6ab053d77e29
"""
	boundary_condition!(residual, u, p, t)

The residual penalty at the boundaries
"""
...
# ╔═╡ c77abd83-4b6c-405f-87c8-3aa43a3a2a81
"""
	problem_2PBV(u0)

Two point boundary value problem (brute force approach)
"""
problem_2PBV(u0) = TwoPointBVProblem(boundary_layer_problem!,
	boundary_condition!,
	u0,
	tspan)

# ╔═╡ 499d85e3-77d6-456b-86e0-d8069ba111fc
md"## Naive Solver Solutions with Predetermided Initial Values"

# ╔═╡ 8b322069-090e-48d4-9240-8b06dccfbe8e
begin
	local opts = (reltol = 1e-8, abstol = 1e-10)
	local alg_opts = (thread = OrdinaryDiffEq.True(),)
	B1_naive = solve(problem_2PBV([1.0, 0.99990238]), Shooting(Tsit5(;alg_opts...)); opts...);
	M_naive = solve(problem_2PBV([1.0, 0.9051]), Shooting(Tsit5(;alg_opts...)); opts...)
	B0_naive = solve(problem_2PBV([1.0, -10.6942]), Shooting(Tsit5(;alg_opts...)); opts...)
end;

# ╔═╡ d5fc0a3e-2553-428a-bd5b-62b687ef17bb
function figure_numerical_naive_solutions()
	f = Figure()
	ax = Axis(f[1,1], title = L"Numerical naive solutions $B0$, $M$ and $B1$ with $\varepsilon = 0.1$", xlabel = L"x", ylabel = L"y")
	for (sol, name) in [(B0_naive, L"y_\text{B0}(x)") , (M_naive, L"y_\text{M}(x)"), (B1_naive, L"y_\text{B1}(x)")]
		lines!(ax, sol.t, sol[1,:], label = name)
	end
	axislegend(ax)
...
# ╔═╡ 12ce97b7-42b7-4e34-be36-6c4fc49f71f3
with_backend(figure_numerical_naive_solutions, CairoMakie)

# ╔═╡ e4b6678b-5aa2-49a7-a945-81f0414b97b7
with_backend(figure_numerical_naive_solutions, GLMakie)

# ╔═╡ f2e10d7f-6450-4ef4-b73c-ba143eef9122
md"""
!!! danger
	Off course it is the same since we are using the same algorithm `Tsit5` and we have the same initialization. That means that we are on the same precission after just one iteration of the integrator. But what would happen if we would inicialize with different values?
"""

# ╔═╡ e36d96c8-361c-40b1-a1fa-667593267e0f
md"## Naive Solver Solution for Different Initial Values"

# ╔═╡ 726c7dd4-1c4c-44ed-a3cd-3741234dad5d
function figure_numerical_naive_solutions_with_IV(num_sols = 20)
	opts = (reltol = 1e-8, abstol = 1e-10)
	alg_opts = (thread = OrdinaryDiffEq.True(),)

	f = Figure()
	ax = Axis(f[1,1])
	
	for y′0 in range(1,0, num_sols)[2:end]
		naive = solve(problem_ODE([1.0,y′0]), Tsit5(;alg_opts...); opts...)
		lines!(ax, naive.t, naive[1,:], label = LaTeXString("y(0) = $(round(y′0, sigdigits = 2))"), color = Makie.wong_colors()[1])
	end
	axislegend(ax)
	f
end

# ╔═╡ 49dc5934-a6b1-4454-9c1c-40305b1f60f5
with_backend(function() figure_numerical_naive_solutions_with_IV(500) end, CairoMakie)

# ╔═╡ 8dac6032-2c6f-413e-81e9-2ab723626199
with_backend(function() figure_numerical_naive_solutions_with_IV(100) end, GLMakie)

# ╔═╡ 702114b2-1ee8-4935-8264-ae850622139b
md"# Phase Portrait Analysis"

# ╔═╡ b49da2eb-44e4-4972-a579-e8e4fa231daf
md"## Making an intuition about the Phase Portrait"

# ╔═╡ b359ad57-48d3-4ca7-871b-ef802c6f9812
function figure_phase_portrait_arrows()
	arrow_number = 25
	ys = LinRange(-2, 2, arrow_number)

	lengthscale = 0.01
	
	f = Figure()
	ax = Axis(f[1,1], xlabel = L"y", ylabel = L"z", title = L"Phase Portrait Arrows $(y', z')$")
...
# ╔═╡ b2b79530-0b3f-4c55-b041-721732d41262
with_backend(figure_phase_portrait_arrows, CairoMakie)

# ╔═╡ d3c15c8b-fb5c-45c7-9669-c3c1b2b10cfd
with_backend(figure_phase_portrait_arrows, GLMakie)

# ╔═╡ 77af5930-1293-48e1-8750-a6f41549379e
md"## Tracing a path of a variable in "

# ╔═╡ 00ef8f06-4f5e-47a6-afbd-79f459f4263c
function figure_trace_phase_portrait(initial_values)
	
end

# ╔═╡ 64736d3d-9167-4c74-b96c-ccde3b9805f7
md"## Solutions in the Phase Portrait"

# ╔═╡ 2749a74e-13c4-4686-81c1-e96aff0c99af
function figure_phase_plane_solutions()
	...
	for (i, (ax, lab, sol)) in enumerate(zip(axes, [L"B0", L"M", L"B1"], [B0, M, B1]))
		ax.title[] = lab
		vlines!(ax, [-1,1]; linestyle = :dash, color = Makie.wong_colors()[4], linewidth = 1)
		lines!(ax, sol[1,:], sol[2,:], color = Makie.wong_colors()[i])
		scatter!(ax, sol[1, [1,end]], sol[2,[1,end]], color = Makie.wong_colors()[i])
	end
...
# ╔═╡ 5ca1788b-3d8e-4504-a752-dfab6a103d56
with_backend(figure_phase_plane_solutions, CairoMakie)

# ╔═╡ f81ad2e4-99e2-41e1-b196-787340e9a674
with_backend(figure_phase_plane_solutions, GLMakie)
...
manifest_format = "2.0"
project_hash = "4b70225df6ba5618a6ea2d797d5574dfeaaed7e1"
...
git-tree-sha1 = "8236ce4eda9837d15bab49573bba16ba0652b486"
